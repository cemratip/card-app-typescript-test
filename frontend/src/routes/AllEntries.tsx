            ...
            {entries.map((entry: Entry, index: number) => {
                return(
                  <div id={entry.id} key={index}
                      ...
                      <section
                        className="flex items-center flex-col md:flex-row pt-2 md:pt-0 space-x-5">
                          <div>
                              <p className="font-bold">Created</p>
                              <time
                                className="text-right text-sm md:text-lg">{new Date(entry.created_at.toString()).toLocaleDateString()}</time>
                          </div>
                          <div>
                              <p className="font-bold">Scheduled</p>
                              <time
                                className="text-right text-sm md:text-lg">{new Date(entry.scheduled_at.toString()).toLocaleDateString()}</time>
                          </div>
                      </section>
