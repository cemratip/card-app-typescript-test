      ...
      <section className="flex justify-center flex-col w-fit ml-auto mr-auto mt-10 gap-5 bg-gray-300 p-8 rounded-md">
        ...
        <div className="space-y-2">
          ...
          <input className="p-3 rounded-md w-full" type="date" name="scheduled_at"
                 value={(new Date(newEntry.scheduled_at)).toISOString().split('T')[0]} onChange={handleInputChange} />
        </div>
        <button onClick={(e) => {
          handleSend(e)
        }} className="bg-blue-400 hover:bg-blue-600 font-semibold text-white p-3 rounded-md">Create
        </button>
      </section>
    )
}